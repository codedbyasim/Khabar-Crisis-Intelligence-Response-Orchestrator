...
# ORCHESTRATOR
# ──────────────────────────────────────────────
class KhabarOrchestrator:
    def __init__(self, api_key: str = None):
        self.memory_block = SharedMemoryBlock()

        # Initialize Gemini-powered LLM client
    ...
    async def process_incident(self, raw_signal: RawCrisisSignal):
        ...
        for attempt in range(max_retries):
            try:
                ...
                detection_result = await self.detection_agent.process_signal(raw_signal)
        ...
        for attempt in range(max_retries):
            try:
                ...
                analysis_result = await self.analysis_agent.process_analysis(analysis_payload)
        ...
        for attempt in range(max_retries):
            try:
                ...
                planning_result = await self.planning_agent.process_plan(planning_payload)
